...
public class SampleModeratorProtocol implements AgentProtocols{
...
public SampleModeratorProtocol(){};		
	...
	public ActiveConversation initialiseProtocols(String agentName,RuntimeConversation conv, 
        ...
        // Adds a protocol to the protocol queue
		   ...
		   if (conv.getInteraction().getId().equals("FirstSessionInt0") && conv.getPlayedRole().equals("Moderator")){
		   DefaultCommControl dcc=new ModeratorFirstSessionInt0DefaultCommControl(
                        		   conv.getConversationID(),msr,lr);
			sb = new ModeratorFirstSessionInt0StateBehavior(
			agentName,
				msr,msu,
					conv,                           		
				   conv.getPlayedRole(),
  ...
  public AgentExternalDescription[] getInteractionActors(final String interaction, final String orgid, final YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
    	return getInteractionActorsWithAdapter(interaction, new AdapterYP(){
  	public DFAgentDescription[] getAgents(String role)  throws FIPAException {return yp.getAgents(orgid, role); };  	
  	});
  }
  
  public AgentExternalDescription[] getInteractionActors(final String interaction, final String orgid, final String groupid, final YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{
    	return getInteractionActorsWithAdapter(interaction, new AdapterYP(){
  	public DFAgentDescription[] getAgents(String role)  throws FIPAException {return yp.getAgents(orgid, groupid, role); };  	
  	});
  }
  
  public AgentExternalDescription[] getInteractionActors(String interaction, final YellowPages yp) throws ingenias.jade.exception.NoAgentsFound{  
  	return getInteractionActorsWithAdapter(interaction, new AdapterYP(){
  	public DFAgentDescription[] getAgents(String role)  throws FIPAException {return yp.getAgents(role); };  	
  	});
  }
 
 
	
	        private AgentExternalDescription[] getInteractionActorsWithAdapter(String interaction, AdapterYP yp) throws ingenias.jade.exception.NoAgentsFound{
              AgentExternalDescription[] result=null;
             
               if (interaction.equals("FirstSessionInt0")){
                ...
                Vector<AgentExternalDescription> cols=new Vector<AgentExternalDescription>();
                 ...
                 String cardinality="1__*";
					...
					DFAgentDescription[] agents=null;		 			
		 			agents=yp.getAgents("Participant");
                   if (agents==null || agents.length<=0)
                      throw new ingenias.jade.exception.NoAgentsFound("Could not find an agent playing the role Participant");
                   if (cardinality.equals("1") || cardinality.equals(""))
                    cols.add(new AgentExternalDescription(agents[0].getName(),"Participant"));
                    else
                	   if (cardinality.equals("1__*"))
						   for (int k=0;k<agents.length;k++)
                			  cols.add(new AgentExternalDescription(agents[k].getName(),"Participant"));
	       ...
	       Vector playedRoles=new Vector();
	        DFAgentDescription dfd=null;
                ...
                if (protocol.equals("FirstSessionInt0")){
		dfd.setName(agentID);
