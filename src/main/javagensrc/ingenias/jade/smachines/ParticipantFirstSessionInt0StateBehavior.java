...
public class ParticipantFirstSessionInt0StateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public ParticipantFirstSessionInt0StateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("waiting for enable","DefineGoalNTIU1");
			    
			    
			    
			      // Receiving a message    
			      smf.add("DefineGoalNTIU1", "waiting for DefineGoalNTIU1");
			      
			      // Next states after receiving "DefineGoalNTIU1"
			      
			       smf.add("waiting for DefineGoalNTIU1","ShareConceptNTIU2");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("ShareConceptNTIU2", "endShareConceptNTIU2");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "FirstSessionInt0","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("DefineGoalNTIU1") ) {  // State changed by other agent and upted
    //in the parent class
      ...
      if ("1".equals("n")){
         try{
          Vector<AID> receivers=this.getActor("Moderator");    
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"DefineGoalNTIU1",
                   "FirstSessionInt0",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("DefineGoalNTIU1");
      this.addState("waiting for DefineGoalNTIU1");
      this.notifyStateTransitionExecuted("DefineGoalNTIU1", "waiting for DefineGoalNTIU1");
  } 
  ...
  if (this.isState("ShareConceptNTIU2")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("ShareConceptNTIU2",optionsA,this)){
           ...
           "ShareConceptNTIU2","FirstSessionInt0",this.getContentForNextMessage(), this.getPlayedRole());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("ShareConceptNTIU2", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("ShareConceptNTIU2", "ABORTED");
    	  } else  {
