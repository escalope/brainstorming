...
public class ModeratorFirstSessionInt0StateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public ModeratorFirstSessionInt0StateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "DefineGoalNTIU1");
			    
			    
			      // Receiving a message    
			      smf.add("ShareConceptNTIU2", "waiting for ShareConceptNTIU2");
			      
			      // Next states after receiving "ShareConceptNTIU2"
			      
			       smf.add("waiting for ShareConceptNTIU2","endShareConceptNTIU2");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("DefineGoalNTIU1", "ShareConceptNTIU2");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
  	try {  	  
      String[] options=new String[]{"DefineGoalNTIU1"};
      AID[] actors=null;
      Vector<AID> actorsv=new Vector<AID>();
      Vector<String> rolesv=new Vector<String>();
      
      actorsv.addAll(this.getActor("Participant"));
      {      
       Vector<AID> receivers=this.getActor("Participant");      
       for (AID aid:receivers){
        rolesv.add("Participant");
       }
      }
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","FirstSessionInt0",completelistInvolvedActors, this.getPlayedRole());
  ...
  if (this.isState("ShareConceptNTIU2") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endShareConceptNTIU2");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"ShareConceptNTIU2",
                   "FirstSessionInt0",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("ShareConceptNTIU2");
      this.addState("waiting for ShareConceptNTIU2");
      this.notifyStateTransitionExecuted("ShareConceptNTIU2", "waiting for ShareConceptNTIU2");
  } 
  ...
  if (this.isState("DefineGoalNTIU1")) {
     ...
     try {
      ...
      options.add("ShareConceptNTIU2");      
      
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("DefineGoalNTIU1",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "DefineGoalNTIU1","FirstSessionInt0",this.getContentForNextMessage(), this.getPlayedRole());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("DefineGoalNTIU1", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("DefineGoalNTIU1", "ABORTED");
    	  } else  {
  ...
  if (this.isState("endShareConceptNTIU2")) {
