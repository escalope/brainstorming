...
import ingenias.exception.NotFound;
  ...
  public class ParticipantFirstSessionInt0DefaultCommControl extends DefaultCommControl{
  ...
  public ParticipantFirstSessionInt0DefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("fake_ShareConceptNT_output_for_task_CollectDrawingsNT");
     
     lw.addDeletionLockExpectedType("ConceptSketch");
    ...
    // Facts that cannot be removed because they must be sent
     ...
     locks.add("fake_ShareConceptNT_output_for_task_CollectDrawingsNT");
     
     locks.add("ConceptSketch");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("ShareConceptNTIU2") &&
    	 sb.isState("ShareConceptNTIU2")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_ShareConceptNT_output_for_task_CollectDrawingsNT").isEmpty();
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ConceptSketch").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("ShareConceptNTIU2");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"fake_ShareConceptNT_output_for_task_CollectDrawingsNT");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("fake_ShareConceptNT_output_for_task_CollectDrawingsNT");
	       lockProcessed("fake_ShareConceptNT_output_for_task_CollectDrawingsNT");
           //MainInteractionManager.log("Removing lock fake_ShareConceptNT_output_for_task_CollectDrawingsNT",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ConceptSketch");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("ConceptSketch");
	       lockProcessed("ConceptSketch");
           //MainInteractionManager.log("Removing lock ConceptSketch",this.getAgent().getLocalName()+"-"+sb.getConversationID());
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for DefineGoalNTIU1")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("DefineGoalNTIU1")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for DefineGoalNTIU1");           
    	   //try {
